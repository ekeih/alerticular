...
async def handle_alerts(message: types.Message) -> None:
    ...
    await message.reply(emojize(text), parse_mode="HTML", disable_web_page_preview=True)
...
async def send_alert(chat: str, alert: JSONType) -> None:
    ...
    await bot.send_message(chat, emojize(message), parse_mode="HTML", disable_web_page_preview=True)
